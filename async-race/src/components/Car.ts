...
class Car {
  ...
  private createCarImage(data: ICar): CreateElement {
    const car = new CreateElement("div", "car-image");
    car.getElement().setAttribute("id", `${data.id}`);
    const createImage = (color: string): string => {
    ...
    car.getElement().innerHTML = createImage(data.color);
    return car;
  }

  render(parentElement: HTMLElement, data: ICar): void {
    parentElement.append(this.createCarImage(data).getElement());
  }
}
